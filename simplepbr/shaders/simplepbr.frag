...
#ifdef ENABLE_SHADOWS
#ifdef USE_330
        float shadowCaster = textureProjSoft(p3d_LightSource[i].shadowMap, v_shadow_pos[i], 0.0001, shadow_blur);
#else
...
#endif
        if(force_shadow > -0.00000001){
            shadowCaster = force_shadow;
        }
